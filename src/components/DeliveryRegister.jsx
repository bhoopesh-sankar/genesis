...
export default function Deliverymanregister() {
    ...
    useEffect(() => {
        document.title = "DeliveryMan Register";
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        if (details.name === '' || details.email === '' || details.password === '' || details.confirmPassword === '' || details.phone === '' || details.address === '' || aadhar === null || drivingLicense === null) {
            setError(true);
        }
            ...
            if (connected === false) {
                ...
                const res = await fetch("https://agro-blockchain-aa2003-default-rtdb.firebaseio.com/deliveryManDetails.json",
                    {
                        method: 'POST',
                        headers: {
                ...
                const aadharref = ref(storage, `DrivingLicense/${aadhar.name + v4()}`)
                await uploadBytes(aadharref, aadhar).then(() => {
                    alert("file uploaded successfully");
                });
                const drivingLicenseref = ref(storage, `DrivingLicense/${aadhar.name + v4()}`)
                await uploadBytes(drivingLicenseref, drivingLicense).then(() => {
                    alert("file uploaded successfully");
                });
                setSubmitted(true);
            }
    ...
    return (
        <div>
            <div>
                <h1 style={{ paddingTop: '100px', paddingLeft: '585px', fontSize: '40px', fontWeight: 'bold', }}>DELIVERY MAN REGISTRATION</h1>
            </div>
            <div style={wholepage}>
                <div style={formbody}>
                    <form>
                        ...
                        <div style={informbody}>
                            <label style={dlicenseCss}>Driving License Card</label>
                            <input onChange={handleDrivingLicense} style={input} type="file" />
                        </div>
                        <br />
                        <br />
            ...
            rounded-full shadow-md hover:bg-green-700" type="submit">
                        ...
                        <h1 style={{ paddingLeft: '40px', fontSize: '20px', fontWeight: 'bold', }}>Already have an account. Click <a href='deliverymanLogin' style={{ color: 'blue', }}>here</a> to log in</h1>
                    </form>
